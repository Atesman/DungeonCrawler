...
func _ready():
	...
	combat_manager.start_combat(turn_order)


func load_player():
	var player_combat_scene = load("res://scenes/combat/PlayerCombatScene.tscn").instantiate() # holds sprite and recognizes click events
	...
	#character_overlay.create_bar_for(player_ref)
	#character_overlay.create_ap_display_for(player_ref)
	#character_overlay.create_def_display_for(player_ref)
...
func load_enemies():
	...
	for enemy_character in enemies_ref:
		...
		#character_overlay.create_bar_for(enemy_character)
		#character_overlay.create_ap_display_for(enemy_character)
		#character_overlay.create_def_display_for(enemy_character)
